...
class newAlarm extends StatefulWidget {
  final Alarm? aux;
  const newAlarm({Key? key,required this.aux}) : super(key: key);

  @override
  State<newAlarm> createState() => newAlarmState();
}
...
class newAlarmState extends State<newAlarm> {
  ...
  Future<void> _notificationManager(String body) async {
    NotificationService.deleteAlarmByKey(idAlarm);
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
          primary: isDark ? darkMainCards : lightMainCards,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(15),
            side: BorderSide(
              width: 1.5,
        ...
        onPressed: () async {
          String? _tempSound = await Navigator.of(context).push(_createRoute(sound)) as String?;
          if (_tempSound != null) {
            sound = _tempSound;
          }
